...
```
1. Hinweis: Wenn bereits eine `.local_dev_state` existiert, wird der erneute Start absichtlich verhindert.
2. Bitte zuerst `make localcleanup` ausführen.

3. Optional (nicht empfohlen): erzwingen mit `FORCE=1 make local-release-test [DEVICE-IP]`.
